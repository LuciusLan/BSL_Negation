...
class DiceLoss(torch.nn.Module):
    ...
    def forward(self, input_: torch.Tensor, target: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        logits_size = input_.shape[-1]

        if logits_size != 1:
            loss = self._multiple_class(input_, target, logits_size, mask=mask)
        else:
            loss = self._binary_class(input_, target, mask=mask)
    ...
    def _multiple_class(self, input_, target, logits_size, mask=None):
        flat_input = input_
        flat_target = F.one_hot(target, num_classes=logits_size).float() if self.index_label_position else target.float()
        flat_input = torch.nn.LogSoftmax(dim=-1)(flat_input) if self.with_logits else flat_input
    ...
    def _binary_class(self, input_, target, mask=None):
